...
class MediaCategoryLibrary {
	...
	*/
	function init() {
		...
		if ( ! isset( $this->settings_data['rewrite_url'] ) ) $this->settings_data['rewrite_url'] = 'mediacat-library';
		if ( ! isset( $this->settings_data['taxonomy_name'] ) ) $this->settings_data['taxonomy_name'] = 'media-category';
		if ( ! isset( $this->settings_data['title'] ) ) $this->settings_data['title'] = 'Media Category Library';
		if ( ! isset( $this->settings_data['posts_per_page'] ) ) $this->settings_data['posts_per_page'] = 20;
		...
		// update counts when images updated
    ...
    */
	function edit_attachment( $post_id ) {
		...
		foreach ( $terms as $t ) {
			$sql = "SELECT COUNT(*) FROM " . $wpdb->term_relationships . " WHERE term_taxonomy_id = " . $t->term_id;
			$count = $wpdb->get_var( $wpdb->prepare( $sql ) );
			$wpdb->update( $wpdb->term_taxonomy, array( 'count' => $count ), array( 'term_taxonomy_id' => $t->term_id ) );
		}
	}
    ...
    */
	function add_media_category_filter() {
		...
		global $wp_query;
		if ( 'upload' == $screen->id ) {
			if ( ! isset( $wp_query->query[$this->settings_data['taxonomy_name']] ) ) $wp_query->query[$this->settings_data['taxonomy_name']] = '';
    ...
    */
	function convert_media_category_to_taxonomy_term_in_query($query) {
		...
		$screen = get_current_screen();
		$qv = &$query->query_vars;
		if ( ! isset( $qv[$this->settings_data['taxonomy_name']] ) ) {
			$qv[$this->settings_data['taxonomy_name']] = '';
		}
	...
	*/
	function media_category_content() {
		if ( ! isset( $_REQUEST['media-categories'] ) ) $_REQUEST['media-categories'] = array();
		ob_start();
...
<?php
	...
	*/
	function get_mediacat_library_pagination( $total_pages, $page, $frontend = false ) {
		...
		if ( ! isset( $_REQUEST['media-categories'] ) ) $_REQUEST['media-categories'][] = 'none';
	...
	*/
	function get_mediacategory_shortcode( $atts = array() ) {
		...
		if ( ! isset( $atts['orderby'] ) ) $atts['orderby'] = array();
		if ( ! isset( $atts['order'] ) ) $atts['order'] = array();
		if ( ! isset( $atts['returnposts'] ) ) $atts['returnposts'] = '';
